...
def gpt_scoring():
    ...
    ####################
    #Path設定
    TESSERACT_PATH = 'C:\\Users\\nakam\\AppData\\Local\\Programs\\Tesseract-OCR' #インストールしたTesseract-OCRのpath
    TESSDATA_PATH = 'C:\\Users\\nakam\\AppData\\Local\\Programs\\Tesseract-OCR\\tessdata' #tessdataのpath

    os.environ["PATH"] += os.pathsep + TESSERACT_PATH
    os.environ["TESSDATA_PREFIX"] = TESSDATA_PATH

    # 身元証明書のjson読み込み
    credentials = service_account.Credentials.from_service_account_file('C:\\Users\\nakam\\Downloads\\composite-dream-388915-94b83c5c67e2.json')
    ...
    file_name = os.path.abspath('C:\\Users\\nakam\\pyocr\\test_images\\science_answer_sheet01.PNG')
    ...
    #解析画像読み込み(雨ニモマケズ)
    test_img = Image.open('C:\\Users\\nakam\\pyocr\\test_images\\science_test.png') #他の拡張子でもOK
    #解析画像読み込み(雨ニモマケズ)
    answer_img = Image.open('C:\\Users\\nakam\\pyocr\\test_images\\science_answer_sheet01.PNG') #他の拡張子でもOK
    ...
    for res in txt_pyocr:
        for key, value in answer_dic.items():
            print(res.content)
            print(res.position)
            print(res.position[0][0])
            print(res.position[0][1])
            print(res.position[1][0])
            print(res.position[1][1])
            if re.compile(key).search(res.content) and value=="正解":
                print((int((res.position[1][0] - res.position[0][0])/2 + res.position[0][0]),int((res.position[1][1] - res.position[0][1])/2 + res.position[0][1])))
